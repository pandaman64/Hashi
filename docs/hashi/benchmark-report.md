...
| --- | ---: | ---: | ---: |
| `insert_grow` | 226.534 | 145.346 | 1.559× |
| `find_hit` | 38.746 | 28.921 | 1.340× |
| `find_miss` | **11.263** | 16.750 | **0.672×** |

Hashi の `find_miss` は `Std.HashMap` より 32.8% 高速になった。insert と
hit は引き続き Std が速い。
...
- insert は引き続きRC解放と再ハッシュが支配的。`reserve` 済み測定との分離が必要
